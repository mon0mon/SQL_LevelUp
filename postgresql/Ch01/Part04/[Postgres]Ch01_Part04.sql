...
`
8)저자주_사실 60개 정도 분량의 레코드는, WHERE 구에 기본 키를 지정해도 인덱스 스캔을
사용하지 않고 풀 스캔을 하는 경우도 있습니다.
`
책 125p에 저자주에 비슷한 내용 언급
`
1)저자주_테이블의 레코드 수가 적은 경우 옵티마이저가 인덱스 스캔을 선택하지 않고,
테이블 풀 스캔을 선택한 가능성이큽니다. 이는 테이블 크기가 작을 경우 테이블 풀 스캔과
인덱스 스캔의 성능 차이가 거의 나지 않기 때문입니다.
`
 */
